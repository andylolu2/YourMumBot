...
class YourMumModel:
    ...
    # public methods
    def yourmumify(self, text: str, log: bool = True) -> Tuple[List[str], List[float]]:
        """
        ...
        with timer(logger=logger, prefix="CoreNLP latency: "):
            ann = dict(self._corenlp_client.annotate(text))
        for sent in ann["sentences"]:
            ...
            # evaluate toxicity of augmented setence, find the most toxic one
            scores = self._toxic_score(yourmumified, key="toxicity")
            if log:
                logger.debug(f"Detoxify scores: {list(zip(yourmumified, scores))}")
    ...
    async def async_yourmumify(
        ...
        return await loop.run_in_executor(
            None, functools.partial(self.yourmumify, text, log=log)
        )
    ...
    def _toxic_score(self, texts: List[str], key: str) -> List[float]:
        ...
        """
        with timer(logger=logger, prefix="Detoxify latency: "):
    ...
    def _correct_grammar(self, text: str) -> str:
        ...
        """
        with timer(logger=logger, prefix="Corrector latency: "):
